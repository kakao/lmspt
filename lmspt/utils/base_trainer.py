...
class BaseTrainer(object):
    ...
    def __init__(self, args=None, cfg=None):
        ...
        self.logging_steps = self.cfg.train.logging_steps
        self.eval_steps = self.cfg.train.eval_steps
        self.logger.info(
           f"This experiment will be evaluated every {self.eval_steps} steps"
        )

        # set random seed (incorporate rank so each GPU gets different randomness)
        self._set_random_seed(args.seed + self.accelerator.process_index)
    ...
    def _add_metric_header(self, metrics: dict, header: str) -> dict:
        new_metrics = {}
        for k, v in metrics.items():
            new_metrics[f"{header}/{k}"] = v
        return new_metrics
    ...
    def save_checkpoint(self):
        if self.accelerator.is_main_process:
            ...
            if len(all_ckpts) > keep_last:
                # only keep `keep_last` folder in self.checkpoint_dir, sort by step  "epoch-{:04d}_step-{:07d}"
                all_ckpts = sorted(
            ...
            checkpoint_filename = f"epoch-{self.epoch:03d}_step-{self.step:07d}"
    ...
    def train_loop(self):
        ...
        while self.epoch < self.max_epoch:
            # Update DistributedSampler epoch so shuffle order changes each epoch
    ...
    def get_lr(self, it):
        ...
        if it < self.cfg.train.scheduler.warmup_steps:
            return self.cfg.train.learning_rate * it / self.cfg.train.scheduler.warmup_steps
        # 2) if it > lr_decay_iters, return min learning rate
        total_steps = len(self.train_dataloader) * self.max_epoch
        if it > total_steps:
            return self.cfg.train.scheduler.min_lr
        # 3) in between, use cosine decay down to min learning rate
        decay_ratio = (it - self.cfg.train.scheduler.warmup_steps) / (
            total_steps - self.cfg.train.scheduler.warmup_steps
        )
        assert 0 <= decay_ratio <= 1
        coeff = 0.5 * (1.0 + math.cos(math.pi * decay_ratio))  # coeff ranges 0..1
        return self.cfg.train.scheduler.min_lr + coeff * (
            self.cfg.train.learning_rate - self.cfg.train.scheduler.min_lr
        )
    ...
    def log(self, *args, **kwargs):
        ...
        """
        try:
            if self.step % self.logging_steps == 0 and self.accelerator.is_main_process:
    ...
    def _train_epoch(self):
        ...
        num_finished_batches = 0  # number of sample passes
        ...
        while True:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                if loss is not None:
                    if type(loss) is tuple:
                        stats = self._add_metric_header(loss[1], "Train")
                        self.log(stats, step=self.step)
            ...
            if self.accelerator.sync_gradients:
                ...
                if self.step % self.logging_steps == 0:
                    self.log(
                        {
                            "Step/Epoch": self.epoch +  num_finished_batches / len(self.train_dataloader),
                ...
                if self.step > 0 and self.step % self.eval_steps == 0:
                    self._valid_epoch(header="Valid")
    ...
    def _valid_epoch(self, header="Valid"):
        r"""Testing epoch. Should return average loss of a batch (sample) over
        one epoch. See ``train_loop`` for usage.
        """

        self.logger.info(f"Start {header} epoch...")
        ...
        if hasattr(model_module, "reset_vocab_usage"):
            model_module.reset_vocab_usage()
        ...
        for batch in tqdm(
            ...
            local_metrics = self._valid_step(batch)
            gathered_metrics = self.accelerator.gather_for_metrics(local_metrics)
            total_samples += self.accelerator.num_processes
            for k, values in gathered_metrics.items():
                v = values.sum()
                if not k in eval_metrics:
                    eval_metrics[k] = v
                else:
                    eval_metrics[k] += v

        for k, v in eval_metrics.items():
            eval_metrics[k] /= total_samples

        if hasattr(model_module, "reset_vocab_usage"):
            model_module.reset_vocab_usage()

        eval_metrics = self._add_metric_header(eval_metrics, header=header)
        self.log(eval_metrics, step=self.step)
        self.logger.info(f"Finish {header} epoch. Elapsed: {time.time() - start_time:.2f}s")
        self.model.train()
    ...
    def _dataloader_worker_init_fn(worker_id):
        """Seed Python random per DataLoader worker to avoid identical augmentations."""
        seed = torch.initial_seed() % (2**32)
        random.seed(seed + worker_id)
        np.random.seed(seed + worker_id)

    def _build_dataloader(self, train_dataloader, valid_dataloader):
        # setup data_loader
        self.logger.info("Building dataset...")
        start = time.monotonic_ns()

        # Inject worker_init_fn if not already set, so each DataLoader worker
        # gets a unique Python random seed (PyTorch only auto-seeds torch/numpy).
        for dl in [train_dataloader, valid_dataloader]:
            if dl.worker_init_fn is None and dl.num_workers > 0:
                dl.worker_init_fn = self._dataloader_worker_init_fn
        ...
        self.logger.info(f"Train data size: {len(self.train_dataloader)}, Valid data size: {len(self.valid_dataloader)}")
    ...
    def __log_parameters(self):
        self.logger.info("-" * 68)
        self.logger.info(f"{'Model':20} | {'Module':30} | {'#Params (M)':>12}")
        for key, value in self.model.items():
            self.logger.info("-" * 68)
            total_params = 0.0
            for i, (name, module) in enumerate(self.model[key].named_children()):
                num_params = sum(p.numel() for p in module.parameters())
                total_params += num_params
                header = ""
                if i == 0:
                    header = key
                self.logger.info(f"{header:20} | {name:30} | {num_params / 1e6:12.3f}")
            self.logger.info(f"{'':20} | {'-' * 45}")
            self.logger.info(f"{'':20} | {'Total':30} | {total_params / 1e6:12.3f}")
        if hasattr(self.cfg, "semantic_model"):
            self.logger.info("-" * 68)
            self.logger.info(f"{'Semantic Model':20} | {'Module':30} | {'#Params (M)':>12}")
            self.logger.info("-" * 68)
            total_params = 0.0
            header = "Semantic Model"
            for i, (name, module) in enumerate(self.cfg.semantic_model["model"].model.named_children()):
                num_params = sum(p.numel() for p in module.parameters())
                total_params += num_params
                self.logger.info(f"{header:20} | {name:30} | {num_params / 1e6:12.3f}")
            self.logger.info(f"{'':20} | {'-' * 45}")
    ...
    def __log_quantizer(self):
        if "generator" in self.model:
            if hasattr(self.model["generator"], "quantizer_setup"):
                from pprint import pformat
                q_setup = self.model["generator"].quantizer_setup
                self.logger.info(pformat(q_setup, indent=4, width=80, compact=False))
